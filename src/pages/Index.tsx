...
const Index = () => {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            // Only add the animation class — the CSS animation (fill-mode: forwards)
            // overrides the opacity-0 cascade value and holds opacity:1 after it ends.
            // Removing opacity-0 separately risks a 1-frame flash before the animation
            // starts its own from-opacity:0 state.
  ...
  const handleNewsletterSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    // Netlify Forms handles the actual submission via the data-netlify attribute.
    // We submit the form natively so Netlify can capture it, then show success.
    ...
    fetch('/', {
      ...
      .catch(() => setSubmitted(true)); // show success regardless — form still collected
  };
  ...
  return (
    <div ref={setupObserver} className="bg-background text-foreground overflow-x-hidden">
      ...
      >
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          >
            ...
            <span className="font-display text-sm tracking-[0.15em] uppercase text-foreground/70">
              Wild Labs
            </span>
          </button>
          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map(({ label, href }) => (
              <button
                ...
                className="font-display text-xs tracking-[0.15em] uppercase text-muted-foreground hover:text-foreground transition-colors duration-200"
              >
                {label}
              </button>
            ))}
          ...
          <div className="md:hidden bg-background/98 border-b border-border px-6 pb-6 flex flex-col gap-1">
            {navLinks.map(({ label, href }) => (
              <button
                key={label}
                onClick={() => scrollTo(href)}
                className="font-display text-xs tracking-[0.15em] uppercase text-muted-foreground hover:text-foreground transition-colors text-left py-3 border-b border-border/40 last:border-0"
              >
                {label}
              </button>
            ))}
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center">
          ...
          <div className="reveal opacity-0">
            ...
            <p className="text-muted-foreground text-base md:text-lg leading-relaxed mb-4 font-body">
              Wild Labs is an independent game studio focused on bold ideas, rapid experimentation,
              and building games players obsess over.
            </p>
            <p className="text-muted-foreground text-base md:text-lg leading-relaxed font-body">
              We believe the best games come from curiosity, iteration, and teams who dare to push
              boundaries.
            </p>
          </div>
        </div>
          ...
          {[
            {
              ...
              desc: 'We test fast, learn fast, and follow what players love.',
            },
            {
              icon: Heart,
              title: 'Player Obsession',
              desc: 'We design experiences players return to again and again.',
            },
            {
              icon: Users,
              title: 'Small Teams. Big Impact.',
              desc: 'Lean teams move faster and build better.',
            },
            ...
            >
              <Icon
                className="w-8 h-8 text-primary mb-6 transition-colors group-hover:text-primary"
                strokeWidth={1.5}
              />
      ...
      <section id="team" className="py-32 px-6 md:px-16 lg:px-24">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16 reveal opacity-0">
            <p className="text-primary/60 font-display text-xs tracking-[0.3em] uppercase mb-4">
              The Pack
            </p>
            ...
            {teamMembers.map(({ name, title, img }, i) => (
              <div
                ...
                className={`reveal opacity-0 group text-center ${
                  // Centre the last card when it would otherwise sit alone in a 3-col row
                  i === teamMembers.length - 1 && teamMembers.length % 3 === 1
                    ? 'sm:col-start-2'
                    : ''
                }`}
              ...
              >
                ...
                <h3 className="font-display text-sm md:text-base font-semibold tracking-tight">
                  {name}
                </h3>
      ...
      <section
        id="lab"
        className="relative py-32 px-6 md:px-16 lg:px-24 blueprint-grid overflow-hidden"
      >
      ...
      <section className="py-24 px-6 md:px-16 lg:px-24 border-t border-border">
        <div className="max-w-xl mx-auto text-center reveal opacity-0">
          ...
          <p className="text-muted-foreground text-sm leading-relaxed mb-8 font-body">
            No spam. Just secrets — when the time is right.
          </p>
          {submitted ? (
            <p className="text-primary font-display text-sm tracking-[0.15em] uppercase animate-fade-in-up">
            ...
            >
              ...
              <button
                type="submit"
                className="px-8 py-3 border border-primary/50 text-primary font-display text-xs tracking-[0.2em] uppercase transition-all duration-300 hover:glow-cyan hover:bg-primary/10 hover:border-primary whitespace-nowrap"
              >
                Notify Me
              </button>
            </form>
