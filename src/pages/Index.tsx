    ...
    <div ref={setupObserver} className="bg-background text-foreground overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center scanlines overflow-hidden">
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {Array.from({ length: 30 }).map((_, i) => (
            <div key={i} className="absolute w-[2px] h-[2px] rounded-full bg-cyan/40"
              style={{ left: `${Math.random() * 100}%`, top: `${Math.random() * 100}%`, animation: `particle-float ${8 + Math.random() * 12}s linear infinite`, animationDelay: `${Math.random() * 10}s` }} />
          ))}
        </div>
        <div className="absolute w-[500px] h-[500px] rounded-full bg-primary/20 pointer-events-none" style={{ animation: 'glow-pulse 6s ease-in-out infinite', filter: 'blur(80px)' }} />
        <div className="relative z-10 flex flex-col items-center gap-8 px-4">
          ...
          <p className="text-muted-foreground text-base md:text-lg tracking-wide max-w-md text-center font-body">Something powerful is being built behind closed doors.</p>
          <button onClick={() => scrollTo('about')} className="mt-4 px-8 py-3 border border-primary/50 text-primary font-display text-sm tracking-[0.2em] uppercase transition-all duration-300 hover:glow-cyan hover:bg-primary/10 hover:border-primary">Enter the Lab</button>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center">
          <div className="reveal opacity-0 flex justify-center">
            <div className="relative">
              <div className="absolute inset-0 rounded-full bg-primary/10 pointer-events-none" style={{ animation: 'glow-pulse 5s ease-in-out infinite', filter: 'blur(60px)', transform: 'scale(1.5)' }} />
              <img src={wolfLogo} alt="Wild Labs Emblem" className="relative w-48 h-48 md:w-64 md:h-64 object-contain" />
            </div>
          </div>
          <div className="reveal opacity-0">
            <h2 className="font-display text-3xl md:text-4xl font-bold tracking-tight mb-6">A studio built for experiments.</h2>
            <p className="text-muted-foreground text-base md:text-lg leading-relaxed mb-4 font-body">Wild Labs is an independent game studio focused on bold ideas, rapid experimentation, and building games players obsess over.</p>
            <p className="text-muted-foreground text-base md:text-lg leading-relaxed font-body">We believe the best games come from curiosity, iteration, and teams who dare to push boundaries.</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-32 px-6 md:px-16 lg:px-24">
        <div className="max-w-5xl mx-auto grid sm:grid-cols-3 gap-8">
          {[
            ...
            <div key={title} className="reveal opacity-0 group border border-border bg-card p-8 transition-all duration-500 hover:border-primary/40 hover:glow-cyan" style={{ animationDelay: `${i * 0.15}s` }}>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-8 md:gap-12">
            {teamMembers.map(({ name, title, img }, i) => (
              <div key={name} className={`reveal opacity-0 group text-center ${i === teamMembers.length - 1 && teamMembers.length % 3 === 1 ? 'sm:col-start-2' : ''}`} style={{ animationDelay: `${i * 0.1}s` }}>
      ...
      <section id="lab" className="relative py-32 px-6 md:px-16 lg:px-24 blueprint-grid overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto text-center reveal opacity-0">
          ...
          <h2 className="font-display text-3xl md:text-4xl font-bold tracking-tight mb-6 text-glow-cyan">In the Lab</h2>
          <p className="text-muted-foreground text-base md:text-lg leading-relaxed mb-4 font-body">Wild Labs is currently building new game experiences and exploring experimental ideas.</p>
          <p className="text-muted-foreground text-base md:text-lg leading-relaxed font-body">More information will be revealed soon.</p>
      ...
      <section id="contact" className="py-32 px-6 md:px-16 lg:px-24">
        <div className="max-w-2xl mx-auto text-center reveal opacity-0">
          <h2 className="font-display text-3xl md:text-4xl font-bold tracking-tight mb-6">Work With Us</h2>
          <p className="text-muted-foreground text-base md:text-lg leading-relaxed mb-10 font-body">We're always looking for curious builders who want to create something extraordinary.</p>
          <a href="mailto:hello@wildlabs.se" className="inline-block px-8 py-3 border border-secondary/50 text-secondary font-display text-sm tracking-[0.2em] uppercase transition-all duration-300 hover:glow-gold hover:bg-secondary/10 hover:border-secondary">Contact</a>
        </div>
      </section>
      ...
      <section className="py-24 px-6 md:px-16 lg:px-24 border-t border-border">
        <div className="max-w-xl mx-auto text-center reveal opacity-0">
          <p className="text-primary/60 font-display text-xs tracking-[0.3em] uppercase mb-4">Stay in the Loop</p>
          ...
          {submitted ? (
            ...
            <form name="newsletter" method="POST" data-netlify="true" onSubmit={handleNewsletterSubmit} className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <input type="email" name="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="your@email.com" className="flex-1 bg-transparent border border-border px-4 py-3 text-sm font-body text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/60 transition-colors min-w-0" />
              <button type="submit" className="px-8 py-3 border border-primary/50 text-primary font-display text-xs tracking-[0.2em] uppercase transition-all duration-300 hover:glow-cyan hover:bg-primary/10 hover:border-primary whitespace-nowrap">Notify Me</button>
            </form>
          )}
        </div>
      </section>
